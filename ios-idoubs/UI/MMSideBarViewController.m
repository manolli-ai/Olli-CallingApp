//
//  MMSideBarViewController.m
//  idoubs
//
//  Created by Man Huynh on 2/23/18.
...
@interface MMCalloutItemView : UIView
...
@implementation MMCalloutItemView
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        _title.font = [UIFont fontWithName:@"Arial" size:12];
...
- (void)layoutSubviews {
    ...
    self.title.frame = CGRectMake(12.5, inset*2-30, 50, 30);
}
...
@interface MMSideBarViewController ()
...
static MMSideBarViewController *rn_frostedMenu;
@implementation MMSideBarViewController
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors titles:(NSArray *)titles {
    if (self = [super init]) {
        ...
        [self.images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop) {
            MMCalloutItemView *view = [[MMCalloutItemView alloc] init];
            view.itemIndex = idx;
            view.clipsToBounds = YES;
            view.imageView.image = image;
            view.title.text = titles[idx];
            [_contentView addSubview:view];
            
            [self.itemViews addObject:view];
            
            if (self.borderColors && self.selectedIndices && [self.selectedIndices containsIndex:idx]) {
                UIColor *color = self.borderColors[idx];
...
- (void)animateSpringWithView:(MMCalloutItemView *)view idx:(NSUInteger)idx initDelay:(CGFloat)initDelay {
...
- (void)animateFauxBounceWithView:(MMCalloutItemView *)view idx:(NSUInteger)idx initDelay:(CGFloat)initDelay {
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    [self.itemViews enumerateObjectsUsingBlock:^(MMCalloutItemView *view, NSUInteger idx, BOOL *stop) {
